 ...
 * on 2018/11/14
 */
public class FloatButtonTranslationBehavior extends BaseFloatButtonBehavior {

    public FloatButtonTranslationBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void onNestedScroll(@NonNull CoordinatorLayout coordinatorLayout, @NonNull FloatingActionButton child, @NonNull View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed, int type) {
        ...
        if (dyConsumed > 0 || dyUnconsumed > 0 && child.getVisibility() == View.VISIBLE) {
            translationDownY(child);
        } else {
            translationUpY(child);
        }
    }
